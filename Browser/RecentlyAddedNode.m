...
- (id) init
{
	if((self = [super initWithName:NSLocalizedStringFromTable(@"Recently Added", @"Library", @"")])) {
		_count = 25;
...
- (void) loadStreams
{
	NSPredicate			*predicate			= [NSPredicate predicateWithFormat:@"%K != nil", StatisticsDateAddedKey];
	NSSortDescriptor	*descriptor			= [[NSSortDescriptor alloc] initWithKey:StatisticsDateAddedKey ascending:NO];
...
{
	NSAssert([self canRemoveStream], @"Attempt to remove a stream from an immutable RecentlyAddedNode");	
